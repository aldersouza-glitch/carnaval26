...
import { HelpCircle, Search, Building2, Info, TrendingDown, MousePointerClick } from 'lucide-react';
...
const OthersBreakdownView: React.FC<OthersBreakdownViewProps> = ({ onSelectObservation }) => {
  const othersTotal = UNAVAILABLE_REASONS.find(r => r.reason === 'OUTROS')?.count || 35;
  const [searchTerm, setSearchTerm] = useState('');

  // Agrupamento por Observação para ranking
  const observationRanking = OTHERS_OBSERVATIONS.reduce((acc: any[], curr) => {
    ...
    if (existing) {
      ...
      acc.push({ name: curr.observation, count: curr.count, category: curr.category });
    }
    return acc;
  }, []).sort((a, b) => b.count - a.count);

  const filteredObservations = OTHERS_OBSERVATIONS.filter(obs => 
    obs.observation.toLowerCase().includes(searchTerm.toLowerCase()) ||
    obs.opm.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <div className="bg-white rounded-xl p-6 border border-slate-200 shadow-sm flex flex-col md:flex-row items-center justify-between gap-6">
        <div className="flex items-start gap-4">
          ...
          <div>
            ...
            <p className="text-slate-600 mt-1 max-w-2xl">
              Detalhamento das observações específicas registradas para os casos que não se enquadram nas categorias padrão. Dados baseados na planilha oficial de indisponibilidade.
            </p>
          </div>
        </div>
        <div className="flex flex-col items-center gap-2">
            <div className="text-center px-8 py-4 bg-red-600 rounded-2xl shadow-lg transform hover:scale-105 transition-transform">
                <p className="text-xs text-red-100 uppercase font-bold tracking-widest">Registros Detalhados</p>
            ...
            {onSelectObservation && (
                <div className="flex items-center gap-1.5 text-[10px] font-black uppercase text-red-600 bg-red-50 px-3 py-1 rounded-full border border-red-100">
                    ...
                    Clique no gráfico para pormenorizar
                </div>
            )}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-6">
        ...
        <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm flex flex-col">
          <h3 className="font-bold text-slate-800 uppercase text-xs tracking-widest mb-6 flex items-center gap-2">
            ...
            Ranking de Observações Mais Frequentes
          </h3>
          <div className="w-full h-[400px]">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart
                layout="vertical"
                data={observationRanking.slice(0, 12)}
              ...
              >
                ...
                >
                  ...
                  {observationRanking.slice(0, 12).map((entry, index) => (
                    <Cell 
                        key={`cell-${index}`} 
      ...
      {/* Lista Nominal / Tabela Detalhada */}
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden flex flex-col">
        <div className="p-4 bg-slate-50 border-b border-slate-200 flex flex-col sm:flex-row justify-between items-center gap-4">
          <h3 className="font-bold text-slate-800 uppercase text-xs tracking-widest flex items-center gap-2">
            ...
            Relação Nominal de Observações
          </h3>
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-slate-100">
            <thead className="bg-slate-50">
              <tr>
                <th className="px-6 py-4 text-left text-[10px] font-black text-slate-500 uppercase tracking-widest">Observação (Motivo)</th>
                <th className="px-6 py-4 text-left text-[10px] font-black text-slate-500 uppercase tracking-widest">Categoria</th>
                <th className="px-6 py-4 text-left text-[10px] font-black text-slate-500 uppercase tracking-widest">OPM Originária</th>
                <th className="px-6 py-4 text-center text-[10px] font-black text-slate-500 uppercase tracking-widest">Qtd</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100">
              {filteredObservations.map((item, idx) => (
                <tr 
                    key={idx} 
                    className={`hover:bg-slate-50 transition-colors group ${onSelectObservation ? 'cursor-pointer' : ''}`}
                    onClick={() => onSelectObservation && onSelectObservation(item.observation)}
                >
                  <td className="px-6 py-4">
                    <span className="text-sm font-black text-slate-900">{item.observation}</span>
                  </td>
                  <td className="px-6 py-4">
                    <span className={`px-2 py-1 rounded-full text-[10px] font-bold uppercase ${
                      item.category === 'Jurídico' ? 'bg-red-100 text-red-700' :
                      item.category === 'Função/Cargo' ? 'bg-blue-100 text-blue-700' :
                      item.category === 'Social/Saúde' ? 'bg-green-100 text-green-700' : 'bg-slate-100 text-slate-600'
                    }`}>
                      {item.category}
                    </span>
                  </td>
